...
def get_translation_jacobian(robot, joint_indices):
    ...
    ### YOUR CODE HERE ###
    ee_position = get_ee_transform(robot, joint_indices)[:3, 3]

    for i in range(len(joint_indices)):
        joint_axis = get_joint_axis(robot, joint_indices[i])
        joint_position = get_joint_position(robot, joint_indices[i])
        J[:, i] = np.cross(joint_axis, (ee_position - joint_position))
    ### YOUR CODE HERE ###
    return J
...
def main():
    ...
    while True:
        ...
        if np.linalg.norm(error) < threshold:
            ...
            print("The configuration is: ", q_current)
            break
        
        J = get_translation_jacobian(robot, joint_idx)
        J_pinv = get_jacobian_pinv(J)
        N = get_nullspace_projection(J)
        velocity = repel_from_limits(q_current, joint_limit, beta)
        ...
        for i, delta in enumerate(delta_q):
            ...
            q_current[0, i] = np.clip(q_current[0, i] + delta, lower, upper)
    ### YOUR CODE HERE ###
